...
const completeActionsObject = (actions, admin) => {
    return actions
        ...
        .filter(a => a.name && (admin.options.defaultActions.includes(a.name) ||  (admin.options.enableOperateAction && a.name === "operate") ))
}
const validPermissions = (permissions, admin) => {
    return !admin.authProvider ||
        (Array.isArray(permissions) ? permissions : [permissions])
            .filter(p => admin.store.getters["auth/getPermissions"].includes(p))
}
...
export const getActionTitle = (resource, action, admin) => {
    ...
    return admin.i18n && admin.i18n.te(actionNameKey) ? admin.i18n.t(actionNameKey, {resource: resourceTitle}) : startCase(`${action.name}_${resource.name}`)
}
...
export const resourceNavItems = (
    ...
    let permittedActions = resourceObj.actions
        .filter(a => hasActionPermission(resourceObj, a, admin))

    let actionObj = action && permittedActions.find(a => a.key === action)
    if (!actionObj) {
        let newPageActions = permittedActions
            .filter(a => ["create", "retrieve"].indexOf(a.name) > -1) // create or retrieve
...
export const resourceRoutes = (resource, admin) => {
    return {
        ...
        children: resource.actions.filter(a => a.displayMode === 'page')
